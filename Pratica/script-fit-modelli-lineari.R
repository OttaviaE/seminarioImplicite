...
# la a accentata di felicità rompe le scatole e va tolta 
...
# guardiamo i dati 
head(iat_data)
table(iat_data$participant)
table(iat_data$condition, iat_data$participant)
length(unique(iat_data$participant))
length(unique(iat_data$stimuli))
...
graph_b_ci +  # object containing the starting plot
  ...
  theme_light() +  # define the theme of the plot # change the colors of the conditions 
