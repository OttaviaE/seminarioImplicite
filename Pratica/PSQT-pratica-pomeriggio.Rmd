...
subtitle: "Analizzare i dati IAT con l'approccio classico"
author: | 
  | Ottavia M. Epifania
  | \texttt{ottavia.epifania@unipd.it}
date: '19 Luglio 2024, Padova'
institute: |
...
output: 
  beamer_presentation: 
    ...
    highlight: haddock
header-includes:
    - \AtBeginDocument{\title[Analisi dei dati]{Misurazione Implicita in Psicologia}}
    - \AtBeginDocument{\date[PSQT]{19 Luglio 2024, Padova}}
    - \AtBeginDocument{\author[O.M.E., Ph.D.]{Ottavia M. Epifania, Ph.D}}
    - \AtBeginDocument{\institute[]{Master di II Livello \newline Psicologia quantitativa. Misurazione, valutazione e analisi di variabili}}
    - \usepackage{graphicx} 
    - \usepackage{setspace}
    - \usepackage{tabularx}
...
x <- paste(x, collapse = "\n")
  } else if (!is.null(n <- options$myline) & !is.null(n <- options$out.lines)) {
    x  = xfun::split_lines(x)
    # any lines wider than n should be wrapped
    if (any(nchar(x) > n)) x = strwrap(x, width = n)
 #   x = paste(x, collapse = '\n')
...
\scalebox{.95}{
			...
			\begin{tabular}{p{2cm} p{6cm}p{3cm}}
				\hline
				\emph{D} score & Error inflation & Eliminazione trial $< 400$ms\\\hline
				\emph{D1} & Built-in correction & No \\
				\emph{D2} & Built-in correction & Si \\
				\emph{D3} & Media (risposte corrette) + 2\emph{sd} & No\\
				\emph{D4} & Media (risposte corrette) + 600 \emph{ms} & No \\
				\emph{D5} & Media (risposte corrette) + 2\emph{sd} & Si\\
				\emph{D6} & Media (risposte corrette) + 600 \emph{ms} & Si \\\hline
			\end{tabular}
...
\begin{columns}
    \begin{column}{.50\linewidth}
    \onslide<2-> \textcolor{blue}{Condizione White-Good/Black-Bad (MappingA)} 
    \end{column}
\begin{column}{.50\linewidth}
    \onslide<3-> \textcolor{red}{Condizione Black-Good/White-Bad (MappingB)}    
\end{column}
\end{columns}
```
...
Si selezionano i blocchi di interesse....Eliminando tutti gli altri!

## 

Dal filtro sulla colonna `blockcode`, si selezionano tutti i blocchi
\underline{\textbf{tranne}} \texttt{PracticeWhitegood}, \texttt{TestWhitegood}, \texttt{PracticeWhitebad}, \texttt{TestWhitebad}:
...
Il file \underline{\textbf{deve}} essere salvato in `.csv` con la virgola settata come separatore di colonna
...
```{r results='markup', myline = 70, out.lines= 5}
...
Il partecipante 9999999 è un partecipante "finto" e va eliminato dal data set 
...
A questo punto si hanno 142 soggetti
...
```{r results='markup', myline = 75}
table(data$blockcode)
```

Bisogna togliere un sacco di cose...

## `trialcode`

\scriptsize

```{r results='markup', myline = 80}
...
```{r eval=FALSE}
write.table(data_clean$data_keep, "cleanIAT.csv", 
            sep = ",",       row.names = FALSE)
```
...
```{r}
iat = data_clean$data_keep # selezionare il data set pulito
```
...
d3 = compute_iat(iat, # il data set pulito 
                 Dscore = "d3") # l'algoritmo desidearto
```
...
```{r, eval=FALSE} 
head(d3[, 16:21]) # colonna da 16 a 21
```
...
```{r results='asis'}
print(descript_d(d3, latex = TRUE), comment = FALSE)
```
...
## Ci siamo dimenticati di `r names(data_clean)[3]`...?

\footnotesize

```{r out.lines=13, myline = 60}
...
In modo che ogni soggetto sia su una riga singola:
...
d3complete = merge(d3, # data set con i D score 
              ...
              by = "participant") # id della variabile per unire
...
## Visualizzare le correlazioni: `corrplot`

Il pacchetto `corrplot` permette di visualizzare le correlazioni.

```{r fig.align='center', out.width="60%"}
library(corrplot)
corrplot(cor(d3complete[, 
          c("dscore_d3", "pol1", "pol2")]))
```
